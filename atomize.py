...
TYPES = ['Land', 'Instant', 'Creature', 'Sorcery', 'Planeswalker', 'Enchantment', 'Battle', 'Artifact', 'Kindred', "Permanent"]

BASICS = ["Plains", "Island", "Swamp", "Mountain", "Forest"]

SUBTYPES = ["Blood", "Clue", "Food", "Gold", "Incubator", "Junk", "Map", "Powerstone", "Treasure", "Equipment", "Vehicle", "Siege", "Aura", "Saga", "Role", "Shard", "Cartouche", "Case", "Class", "Curse", "Rune", "Shrine", "Cave", "Desert", "Gate", "Lair", "Locus", "Mine", "Planet", "Power-Plant", "Sphere", "Tower", "Town", "Urza's", "Adventure", "Arcane", "Chorus", "Lesson", "Trap"]
...
#"atomic regex sub"
#   set flag=re.I for case-insensitive
def atsub(reg, name, s, subs, flag=re.NOFLAG):
    while re.search(reg, s, flags=flag):
        text = re.search(reg, s, flags=flag).group()
        subs.append((name,text))
        s = re.sub(reg, name, s, count=1, flags=flag)
    return s
...
class Atomizer:
    ...
    def atomize(self, s):
        ...
        subs = []
        repl = [("TYPE",TYPES), ("SUBTYPE",SUBTYPES), ("CTYPE",CTYPES)]
        for tag, typs in repl:
            flag = re.NOFLAG
            if tag == "TYPE": #hacky...
                flag = re.I
            for typ in typs:
                if typ.lower() in s.lower(): #check before doing slow re.sub
                    s = atsub(r"\b"+typ+r"\b",tag,s,subs,flag)
                    s = atsub(r"\b"+typ+r"s\b",tag,s,subs,flag)
            s = atsub(r"("+tag+" )+"+tag,tag,s,subs)
